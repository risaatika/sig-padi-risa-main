<x-app-layout>
    <div class="bg-gradient-to-t from-blue-600/[.15] via-transparent h-full">
        <div class="max-w-4xl px-4 pt-10 mx-auto sm:px-6 lg:px-8 lg:pt-14">
            <div class="p-4 border border-gray-700 shadow rounded-xl sm:p-7 bg-slate-900">
                <div class="mb-8">
                    ...
                    <p class="text-sm text-gray-400">
                        {{ __('harvest_data.create_description') }}
                    </p>
                </div>

                <form action="{{ route('harvest-data.store') }}" method="POST">
                    @csrf

                    <div class="grid grid-cols-12 gap-4 sm:gap-6">
                        <div class="col-span-3">
                            <label for="subdistrict_id" class="inline-block text-sm mt-2.5 text-gray-200">
                        ...
                        <div class="col-span-9">
                            <select name="subdistrict_id" id="subdistrict_id"
                                ...
                                required>
                                <option value="">{{ __('harvest_data.pick_one_subdistrict') }}</option>
                                @foreach ($subdistricts as $subdistrict)
                                    <option value="{{ $subdistrict->id }}" @selected(old('subdistrict_id') === $subdistrict->id)>
                        ...
                        <div class="col-span-9">
                            <input id="harvested-area" type="number" name="harvested_area" step="0.1"
                                class="block w-full px-3 py-2 text-sm text-gray-400 border-gray-700 rounded-lg shadow-sm pr-11 focus:border-blue-500 focus:ring-blue-500 bg-slate-900"
                                value="{{ old('harvested_area') }}" required>
                        </div>
                        ...
                        <div class="col-span-9">
                            <input id="productivity" type="number" name="productivity" step="0.01"
                                class="block w-full px-3 py-2 text-sm text-gray-400 border-gray-700 rounded-lg shadow-sm pr-11 focus:border-blue-500 focus:ring-blue-500 bg-slate-900"
                                value="{{ old('productivity') }}" required>
                        </div>
